...
void MultiMerge(KNNDataManager &data_manager, const string &out_data_path,
  ...
  for (int j = i + 1; j < shards_num; j++) {
    ...
    // OLD DEBUG: Massive debug allocations (~1.5GB per iteration) - commented out for performance
    ...
    if (result_knn_graph_dev == nullptr) {
      cerr << "ERROR: MultiMerge - KNNMergeFromHost returned null for shards " 
           << i << " and " << j << endl;
      // OLD DEBUG: Cleanup code for debug allocations - commented out
      //delete[] debug_host_knngraph;
      //delete[] host_graph_old;
      //delete[] host_graph_new;
      //delete[] host_size_old;
      //delete[] host_size_new;
      //delete[] debug_parameters_shrink_graph;
      //delete[] dbg_new_pre_sort_host;
      //delete[] dbg_old_pre_sort_host;
      //delete[] dbg_new_post_sort_host;
      //delete[] dbg_old_post_sort_host;
      //nvtxRangePop();  // OLD PROFILING: commented out for performance
      multi_merge.unlock();
      exit(-1);
    }
    ...
    // OLD DEBUG: Delete debug allocations - commented out for performance
    //delete[] host_graph_old;
    //delete[] host_graph_new;
    //delete[] host_size_old;
    //delete[] host_size_new;
  ...
  //remove check to have more performance
  /*
  ...
  */
    ...
    //TODO: CHECK IF NO PROBLEM REMOVE THIS SYNC
    //CUDA_ERROR_CHECK(cudaDeviceSynchronize());   // <-- catches runtime faults
