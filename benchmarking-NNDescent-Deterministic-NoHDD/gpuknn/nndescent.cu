...
#include "random_seed_config.cuh"
#include "db_size.cuh"
...
__device__ int RemoveDuplicates(int *nums, int nums_size) {
  ...
  //try to fix it if this could be a problem
  ...
  int unique_count = 1; // At least the first element is unique
...
__global__ void PrepareReverseGraphForMerge(
    ...
    int *graph_old_dev, int *oldg_list_size_dev, int *oldg_revlist_size_dev, int block_id,
    const int split_pos) {
  ...
  __shared__ int cache2_size;
  int tx = threadIdx.x;
  //old assignement for when all blocks were used in parallel
  //size_t list_id = blockIdx.x;
  size_t list_id = block_id; // Use the block_id passed as an argument
  //new assignement for when only one block is used in parallel
  size_t knng_base_pos = list_id * NEIGHB_NUM_PER_LIST;
  size_t nn_list_base_pos = list_id * (MERGE_SAMPLE_NUM * 2);
  if (tx == 0) {
    cache1_size = newg_list_size_dev[list_id];
...
__global__ void ShrinkGraphForMerge(int *graph_new_dev, int *newg_list_size_dev,
                                    int *newg_revlist_size_dev,
                                    int *graph_old_dev, int *oldg_list_size_dev,
                                    int *oldg_revlist_size_dev, int *dbg_new_pre_sort, int *dbg_old_pre_sort, int *dbg_new_post_sort, int *dbg_old_post_sort) {
  ...
  size_t nn_list_base_pos = list_id * (MERGE_SAMPLE_NUM * 2);
  int lane_id = tx % WARP_SIZE;
  const int  K   = MERGE_SAMPLE_NUM;       // 12
  const int  P   = K * 2;                  // 24
  size_t dbg_base = list_id * P;           // where this node starts in dbg arrays
  ...
  __syncthreads();
      ...
      /* ========== DEBUG SNAPSHOT 1 : caches AFTER merge, BEFORE pruning ======== */
    for (int p = tx; p < list_new_size; p += blockDim.x)
        dbg_new_pre_sort[dbg_base + p] = new_elements_cache[p];
    for (int p = tx; p < list_old_size; p += blockDim.x)
        dbg_old_pre_sort[dbg_base + p] = old_elements_cache[p];
    __syncthreads(); 
    /* ======================================================================== */
  ...
  __syncthreads();
      ...
      /* ========== DEBUG SNAPSHOT 2 : caches AFTER pruning ======== */
    for (int p = tx; p < newg_list_size; p += blockDim.x)
        dbg_new_post_sort[dbg_base + p] = new_elements_cache[p];
    for (int p = tx; p < oldg_list_size; p += blockDim.x)
        dbg_old_post_sort[dbg_base + p] = old_elements_cache[p];
    __syncthreads(); 
    /* ======================================================================== */
...
void PrepareForUpdateForMerge(int *graph_new_dev, int *newg_list_size_dev,
  ...
  //to search for determinism try to launch this kernel one block at the time instead of in parallel
  for(int i = 0; i < graph_size; i++) {
    PrepareReverseGraphForMerge<<<1, block_size>>>(
      graph_new_dev, newg_list_size_dev, newg_revlist_size_dev, graph_old_dev,
      oldg_list_size_dev, oldg_revlist_size_dev, i, split_pos);
    CUDA_ERROR_KERNEL_CHECK();
    cudaDeviceSynchronize();
  }

  //old launching of a sigle reverse graph kernel
  /*
  ...
  */
  ...
  //Dump the data to host for debugging
...
void ToDevKNNGraph(NNDElement *dev_graph, vector<vector<NNDElement>> host_graph,
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(dev_graph, host_graph_tmp,
...
void GetTestGraph(Graph *graph_new_ptr, Graph *graph_old_ptr,
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(newg_list_size, newg_list_size_dev,
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(oldg_list_size, oldg_list_size_dev,
             (size_t)graph_size * sizeof(int), cudaMemcpyDeviceToHost));
  cuda_status = cudaGetLastError();
  if (cuda_status != cudaSuccess) {
    cerr << cudaGetErrorString(cuda_status) << endl;
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(graph_new, graph_new_dev,
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(graph_old, graph_old_dev,
             (size_t)graph_size * (SAMPLE_NUM * 2) * sizeof(int),
             cudaMemcpyDeviceToHost));
  cuda_status = cudaGetLastError();
  if (cuda_status != cudaSuccess) {
...
__device__ NNDElement GetMinElement(const int *neighbs_id, const int list_id,
  ...
  if(min_elem.label() != LARGE_INT && min_elem.label() > g_db_size) {
    printf("ERROR: label assigned out of bound of the Dataset: %d \n", min_elem.label());
  }
  return min_elem;
}
...
__device__ void InsertToGlobalGraph(NNDElement elem, const int local_id,
  ...
  do {
    ...
    if (loop_flag == 1) {
      ...
      if (pos_to_insert >= 0 && pos_to_insert < NEIGHB_NUM_PER_LIST) {
        int local_pos = block_id * WARP_SIZE + lane_id;
        if (local_pos > pos_to_insert) {
          local_pos++;
        } else if (local_pos == pos_to_insert) {
          if((global_pos_base + local_pos)>= (global_pos_base + NEIGHB_NUM_PER_LIST)) {
            printf("Error: pos_to_insert >= NEIGHB_NUM_PER_LIST\n");
            printf("local_pos: %d, block_id: %d, lane_id: %d\n", local_pos,
              block_id, lane_id);
              printf("global_pos_base=%u  global_id=%u\n",
                (unsigned int)global_pos_base,
    ...
    //__nanosleep(32); // Available on newer NVIDIA architectures
...
__global__ void NewNeighborsCompareKernelForHighDim(
  ...
  for (int bi = 0; bi < vec_block_num; bi++) {
    ...
    for (int i = 0; i < num_it; i++) {
      ...
      int y = tx % MAX_DIM;
      size_t vec_id = neighbors[x];
      shared_vectors[x * MAX_SKEW_DIM + y] =
          vectors[vec_id * VEC_DIM + bi * MAX_DIM + y];
    }
    __syncthreads();
...
//todo try to find a no illegal write
...
__global__ void TiledNewOldNeighborsCompareKernelForMerge(
  ...
  if (tx == 0) {
    ...
    neighbors = (int *)((char *)distances +
                        (num_new_max * num_old_max) * sizeof(float));
    //printf("when allocating memory: num_new_max = %d, num_old_max = %d\n", num_new_max, num_old_max);
  }
  __syncthreads();
  ...
  //printf("After creating lists gnew_base_pos, ...: num_new = %d, num_old = %d\n", num_new, num_old);
  int neighb_num = num_new + num_old;
  if (tx < num_new) {
    neighbors[tx] = graph_new[gnew_base_pos + tx];
  } else if (tx >= num_new && tx < neighb_num) {
  ...
  //printf("After populating neighbors with neighbots[tx]: num_new = %d, num_old = %d\n", num_new, num_old);
  __syncthreads();
...
NNDElement *ReadKNNGraphToGlobalMemory(
  ...
  auto cuda_status =cudaMalloc(&knn_graph_dev, knn_graph.size() * k * sizeof(NNDElement));
  if (cuda_status != cudaSuccess) {
...
float UpdateGraph(NNDElement *origin_knn_graph_dev, const size_t g_size,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&global_locks_dev, g_size * sizeof(int) * NEIGHB_BLOCKS_NUM));
  CUDA_ERROR_CHECK(cudaMemset(global_locks_dev, 0, g_size * sizeof(int) * NEIGHB_BLOCKS_NUM));
  cuda_status = cudaGetLastError();
  ...
  if (calc_between_new_neighbs) {
    if (VEC_DIM < MAX_DIM) {
      ...
      NewNeighborsCompareKernelForHighDim<<<grid_size, VEC_DIM,
                                            shared_memory_size>>>(
          origin_knn_graph_dev, global_locks_dev, vectors_dev, newg_dev,
          newg_list_size_dev, num_new_max);
          CUDA_ERROR_KERNEL_CHECK();  
...
float UpdateGraphForMerge(NNDElement *origin_knn_graph_dev, const size_t g_size,
  ...
  const int num_new_max = GetMaxListSize(newg_list_size_dev, g_size);
  const int num_old_max = GetMaxListSize(oldg_list_size_dev, g_size);
  //printf("num_new_max = %d, num_old_max = %d\n", num_new_max, num_old_max);
...
__global__ void InitKNNGraphIndexKernel(
  ...
  if (random_sequence == 0) {
    // uint64_t label_raw = xorshift64star(pos, global_seed);
    // label = label_raw % (unsigned long long)graph_size;
    // todo try to determinism with xorshift64star with a seed
  ...
  while (label % NEIGHB_BLOCKS_NUM != block_id || label == list_id) {
    ...
    if (iteration_loop > 100) {
      printf("EROR: stuck in trying to find a new label\n");
      break;
    }
  }
...
void InitRandomKNNGraph(NNDElement *knn_graph_dev, const int graph_size,
                        const float *vectors_dev,  bool start_from_random_index,
...
//TODO version with safety checks added for no illegal out of bound access
/*
__global__ void MergeBlocksKernel(NNDElement *knn_graph, const int graph_size) {
  // Sanity check at compile time (if constants)
  static_assert(NEIGHB_BLOCKS_NUM * WARP_SIZE == NEIGHB_NUM_PER_LIST,
                "Invalid config: NEIGHB_BLOCKS_NUM * WARP_SIZE must equal NEIGHB_NUM_PER_LIST");
  ...
  // Load first WARP_SIZE elements
  c_cache[tx] = knn_graph[global_pos_base + tx];
  list_size = WARP_SIZE;

  for (int i = 1; i < NEIGHB_BLOCKS_NUM; i++) {
    // PROTECT against out-of-bounds: skip iteration if it would exceed list bounds
    int read_offset = i * WARP_SIZE;
    if (read_offset >= NEIGHB_NUM_PER_LIST) break;

    list_size = MergeList(
      c_cache, list_size,
      &knn_graph[global_pos_base + read_offset],
      WARP_SIZE,
      tmp_cache
    );
...
void NNDescentRefine(NNDElement *knngraph_dev, const float *vectors_dev,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&graph_new_dev,
             (size_t)graph_size * (SAMPLE_NUM * 2) * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&newg_list_size_dev, (size_t)graph_size * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&newg_revlist_size_dev, (size_t)graph_size * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&graph_old_dev,
             (size_t)graph_size * (SAMPLE_NUM * 2) * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&oldg_list_size_dev, (size_t)graph_size * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&oldg_revlist_size_dev, (size_t)graph_size * sizeof(int)));
  Graph result(vecs_size);

  NNDElement* debug_host_graph = new NNDElement[graph_size * NEIGHB_NUM_PER_LIST];
...
void NNDescentForMerge(NNDElement *knngraph_dev, const float *vectors_dev,
                       const int vecs_size, const int vecs_dim,
                       const int split_pos, NNDElement *debug_host_graph, int *host_graph_old,
                       int *host_graph_new, int *host_size_old,
                       int *host_size_new, int* debug_parameters, int *dbg_new_pre_sort_host, int *dbg_old_pre_sort_host, int *dbg_new_post_sort_host, int *dbg_old_post_sort_host, const int iteration) {
  int k = NEIGHB_NUM_PER_LIST;
  int *graph_new_dev, *newg_list_size_dev, *graph_old_dev, *oldg_list_size_dev;
  int *newg_revlist_size_dev, *oldg_revlist_size_dev;
  int *new_reverse_all_dev, *old_reverse_all_dev;
  int graph_size = vecs_size;
  CUDA_ERROR_CHECK(cudaMalloc(&graph_new_dev,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&oldg_list_size_dev, (size_t)graph_size * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&oldg_revlist_size_dev, (size_t)graph_size * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&new_reverse_all_dev, g_db_size * g_db_size/2 * sizeof(int)));
  CUDA_ERROR_CHECK(cudaMalloc(&old_reverse_all_dev, g_db_size * g_db_size/2 * sizeof(int)));
  Graph result(vecs_size);
  InitRandomKNNGraph(knngraph_dev, graph_size, vectors_dev, false, false);
  auto cuda_status = cudaGetLastError();
  if (cuda_status != cudaSuccess) {
  ...
  for (int t = 0; t < iteration; t++) {
    ...
    PrepareForUpdateForMerge(graph_new_dev, newg_list_size_dev,
                             ...
                             oldg_list_size_dev, oldg_revlist_size_dev,
                             knngraph_dev, split_pos, graph_size, debug_host_graph, t, host_graph_old,
                             host_graph_new, host_size_old, host_size_new, debug_parameters, dbg_new_pre_sort_host, dbg_old_pre_sort_host, dbg_new_post_sort_host, dbg_old_post_sort_host, 
                            new_reverse_all_dev, old_reverse_all_dev);
    auto end = chrono::steady_clock::now();
    float tmp_time =
        (float)chrono::duration_cast<std::chrono::microseconds>(end - start)
            .count() /
...
void NNDescent(NNDElement **knngraph_result_ptr, const float *vectors_dev,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&knngraph_result, (size_t)graph_size * k * sizeof(NNDElement)));
...
vector<vector<NNDElement>> NNDescent(const float *vectors, const int vecs_size,
  ...
  float *vectors_dev;
  CUDA_ERROR_CHECK(cudaMalloc(&vectors_dev, (size_t)vecs_size * vecs_dim * sizeof(float)));
  CUDA_ERROR_CHECK(cudaMemcpy(vectors_dev, vectors, (size_t)vecs_size * vecs_dim * sizeof(float),
             cudaMemcpyHostToDevice));
  NNDElement *knn_graph_dev;
