...
#include "../random_offset_tracker.h"  // include the global counter
...
void DevRNGLongLong(unsigned long long *dev_data, int n) {
  for (int i = 0; i < n; i++) {
    dev_data[i] = (unsigned long long)1;
  }
  ...
  //   curandDestroyGenerator(gen);  // Clean up
}
...
void ToHostKNNGraph(vector<vector<NNDElement>> *origin_knn_graph_ptr,
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(knn_graph, knn_graph_dev,
...
void GenerateRandomKNNGraphIndex(int **knn_graph_index_ptr,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&random_sequence_dev,
             graph_size * neighb_num * sizeof(unsigned long long)));
  DevRNGLongLong(random_sequence_dev, graph_size * neighb_num);
  CUDA_ERROR_CHECK(cudaMalloc(&knn_graph_index, graph_size * neighb_num * sizeof(int)));
...
size_t PredPeakGPUMemory(const int vecs_num, const int vecs_dim, const int k,
                         const int sample_num, const bool thrust_random) {
