    ...
    for (int j = 0; j < it_num; j++) {
      ...
      if (list_id < knngraph_first_size) {
        ...
        new_label =
            ...
            knngraph_first_size;
        // todo try determinims
        //  new_label = xorshift64star(rand_knngraph_pos_base + tx, global_seed)
        //  % knngraph_second_size + knngraph_first_size;
        int cnt = 0;
        while (new_label % NEIGHB_BLOCKS_NUM != i || new_label == list_id) {
          ...
          if (cnt >= 100) {
            ...
            // printf("%d %d\n", new_label,
            //        (int)xorshift64star(new_label + cnt, global_seed) %
            //            knngraph_second_size);
          }
          new_label = xorshift64star(new_label + cnt) % knngraph_second_size +
                      knngraph_first_size;
          // new_label = xorshift64star(new_label + cnt, global_seed) %
          //                 knngraph_second_size +
          //             knngraph_first_size;
        }
      } else {
  ...
  if (list_id < knngraph_first_size) {
    ...
    for (int i = 0; i < it_num; i++) {
      ...
      if (pos < FIRST_HALF_NEIGHB_NUM) {
        ...
        // int new_label =
        //     xorshift64star(knngraph_second_pos_base + pos, global_seed) %
        //     merged_size;
        int cnt = 0;
        while (new_label % NEIGHB_BLOCKS_NUM != i || new_label == list_id) {
          cnt++;
          ...
          // new_label =
          //     xorshift64star(new_label + cnt, global_seed) % merged_size;
        }
  ...
  for (int i = 0; i < NEIGHB_BLOCKS_NUM; i++) {
    ...
    for (int j = 0; j < it_num; j++) {
      ...
      if (list_id < knngraph_first_size) {
        ...
        new_label = xorshift64star(rand_knngraph_pos_base + tx) % merged_size;
        // new_label = xorshift64star(rand_knngraph_pos_base + tx, global_seed)
        // %
        //             merged_size;
        int cnt = 0;
...
void PrepareGraphForMerge(NNDElement **knngraph_dev_ptr,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&knngraph_dev, (size_t)merged_graph_size * NEIGHB_NUM_PER_LIST *
                                sizeof(NNDElement)));
...
void MergeVectors(float **vectors_dev_ptr, float *vectors_first_dev,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&vectors_dev, (size_t)merged_size * VEC_DIM * sizeof(float)));
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(vectors_dev, vectors_first_dev,
  ...
  CUDA_ERROR_CHECK(cudaMemcpy(vectors_dev + (size_t)vectors_first_size * VEC_DIM,
             vectors_second_dev,
...
void KNNMerge(NNDElement **knngraph_merged_dev_ptr, float *vectors_first_dev,
              ...
              NNDElement *knngraph_second_dev, const bool free_sub_data, NNDElement *debug_host_graph, int *dbg_new_pre_sort_host, int *dbg_old_pre_sort_host, int *dbg_new_post_sort_host, int *dbg_old_post_sort_host) {
  ...
  // }
  // out.close();
  //todo eliminate after;
  int total_nodes = vectors_first_size + vectors_second_size;
  ...
  int* debug_parameters_shrink_graph = new int[4];
  ...
  NNDescentForMerge(knngraph_merged_dev, vectors_dev, merged_graph_size,
                    VEC_DIM, vectors_first_size, debug_host_graph, host_graph_old,
                    host_graph_new, host_size_old,
                    host_size_new, debug_parameters_shrink_graph, dbg_new_pre_sort_host, dbg_old_pre_sort_host, dbg_new_post_sort_host, dbg_old_post_sort_host, MERGE_ITERATION);
...
void KNNMergeFromHost(NNDElement **knngraph_merged_dev_ptr,
  ...
  CUDA_ERROR_CHECK(cudaMalloc(&knngraph_first_dev, (size_t)vectors_first_size *
  ...
  float *vectors_dev;
  CUDA_ERROR_CHECK(cudaMalloc(&vectors_dev, (size_t)(vectors_first_size + vectors_second_size) *
                               VEC_DIM * sizeof(float)));
  CUDA_ERROR_CHECK(cudaMemcpy(vectors_dev, vectors_first,
             (size_t)vectors_first_size * VEC_DIM * sizeof(float),
             cudaMemcpyHostToDevice));
  CUDA_ERROR_CHECK(cudaMemcpy(vectors_dev + (size_t)vectors_first_size * VEC_DIM, vectors_second,
             (size_t)vectors_second_size * VEC_DIM * sizeof(float),
             cudaMemcpyHostToDevice));
