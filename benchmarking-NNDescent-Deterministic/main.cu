...
#define K_neighbors 12  // Number of neighbors
...
void TestBuildShardsOnly(int shards) {
  string ref_path = "../data/vectors";
  string result_path = "../results/NNDescent-KNNG.kgraph";
  int K = K_neighbors;
    ...
    if (PREPARE) {
      ...
      if (mode == "full") {
        TestConstructLargeKNNGraph(shards,
                                   n);  // your existing combined build+merge
      } else if (mode == "build_only") {
        BuildShardsOnly(ref_path, result_path, K,
                        shards);  // <- only build shards
      } else if (mode == "merge_only") {

        SetupDbSize(n);
        MergeShardsOnly(ref_path, result_path, K, shards);  // <- only merge
        ToTxtResult(result_path, result_path + ".txt",
                    n);  // <- after merging, save final txt
