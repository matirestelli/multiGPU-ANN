...
class KNNDataManager {
 ...
 public:
  ...
  void LoadAllVectorData() {
    cout << "Loading all vector data into memory (parallel mode)..." << endl;
    
    // Load all shards in parallel using threads
    std::vector<std::thread> threads;
    
    for (int shard_id = 0; shard_id < shards_num_; shard_id++) {
      threads.emplace_back([this, shard_id]() {
    ...
    cout << "All vector data loaded successfully (parallel)" << endl;
  }
  void CheckStatus() {
