...
void ReadGraph(const string &graph_path, NNDElement **knn_graph_ptr,
  ...
  double read_time = read_timer.end();
  {
    std::lock_guard<std::mutex> lock(timing_mutex);
    total_HDD_time += read_time;
  }
}
...
void WriteGraph(const string &graph_path, const NNDElement *knn_graph,
  ...
  double write_time = write_timer.end();
  {
    std::lock_guard<std::mutex> lock(timing_mutex);
    total_HDD_time += write_time;
  }
}
...
void BuildEachShard(KNNDataManager &data_manager, const string &out_data_path,int id) {
  ...
  /*// DEBUG: Stampa i dati del DB dello shard i
    // Salva i dati del DB dello shard i in un file .txt
    const float* vectors = data_manager.GetVectors(i);
    int vecs_num = data_manager.GetVecsNum(i);
    int dim = data_manager.GetDim();
  
    string filename = "vectors_shard_" + to_string(i) + ".txt";
    ...
    for (int v = 0; v < vecs_num; v++) {
      for (int d = 0; d < dim; d++) {
        out_file << vectors[v * dim + d] << " ";
      }
      out_file << endl;
    }
    out_file.close(); */
...
void GenLargeKNNGraph(const string &vecs_data_path, const string &out_data_path,
  ...
  //create timers in which to sum the time of each phase
  /*
  std::atomic<double> total_HDD_time(0.0);
  std::atomic<double> total_Building_time(0.0);
  std::atomic<double> total_Merge_time(0.0);
  std::atomic<double> total_ANNS_time(0.0);
  */
 ...
 for (int s=0; s<iters; s++) {
    ...
    for (int i=0; i<NUM_GPU; i++) {
        ...
        int s_copy = s;
        int i_copy = i;
        int allow_begin_copy = allow_begin;

        threads.push_back(thread([&data_manager, out_data_path, s_copy, i_copy, allow_begin_copy]() {
  ...
  std::cerr << "\n========================================" << std::endl;
  std::cerr << "TIMING RESULTS (HDD-based version):" << std::endl;
  std::cerr << "========================================" << std::endl;
  ...
  //saving timer results in appropriate file
